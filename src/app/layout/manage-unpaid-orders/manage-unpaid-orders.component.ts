...
@Component({
  selector: 'app-manage-unpaid-orders-page',
  templateUrl: './manage-unpaid-orders.component.html',
  styleUrls: ['./manage-unpaid-orders.component.scss'],
  animations: [routerTransition()]
})
export class ManageUnpaidOrdersComponent implements OnInit, OnDestroy, AuthListener {
  items:  Observable<{}[]>;
  itemsArray = [];
  ...
  onUserChanged(user: any) {
    if (user) {
      this.items = this.database.subscribeToUnpaidOrders();
